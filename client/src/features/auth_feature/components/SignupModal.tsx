...
const SignupModal: React.FC<Props> = ({onSwitchToLogin}) => {
  ...
  >
    <div className="h-auto   w-auto md:shadow-lg md:border-2 bg-white py-10 px-8 md:rounded-xl">
       
      <Logo />
      <div className="mt-4 mb-4">
      <h2 className="font-shopify font-bold text-2xl mb-1">Create Stock-Ease account</h2>
      <p className="text-gray-600 text-sm font-shopify1000 ">Start your Stock-Ease journey</p>
        </div>
        <Formik
          ...
          {({ isSubmitting }) => (
            <Form>
              ...
              <div className="flex font-shopify text-sm justify-start mt-10 mb-5">
                <span className='text-gray-600'>Alread a member ? <button onClick={onSwitchToLogin}><span className='text-blue-500'>Login</span></button></span>
              </div>
            </Form>
          )}
        </Formik>
