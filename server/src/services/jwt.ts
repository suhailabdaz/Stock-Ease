...
export default class JwtControllers {
  ...
  isAuthenticated = async (req: Request, res: Response, next: NextFunction) => {
      try {
          ...
          // Verify token
          const decoded = jwt.verify(token, process.env.JWT_SECRET_KEY || "suhail") as DecodedToken;
  ...
  refreshToken = async (req: Request, res: Response) => {
      try {
          ...
          const newRefreshToken = jwt.sign({ id: decoded.userId }, process.env.JWT_REFRESH_SECRET_KEY || 'suhailrefresh', {
              expiresIn: '7d',
          });
  
          const newAccessToken = jwt.sign({ id: decoded.userId }, process.env.JWT_SECRET_KEY || 'suhail', {
              expiresIn: '15m',
          });
